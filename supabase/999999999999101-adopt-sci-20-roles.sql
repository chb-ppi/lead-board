...
create policy "Users can view their profile and colleagues" on public.profiles for select to authenticated using (id = auth.uid() or (public.has_completed_initial_password_change() and team_id is not null and team_id = public.current_team_id()));
